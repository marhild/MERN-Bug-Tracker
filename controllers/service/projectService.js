...
exports.addProjectToPermittedUsers = (projectId, permittedUsers) =>{
  permittedUsers.forEach( async (element, index) =>{
    user = await User.findById(element)
    if(!user.permittedProjects.includes(projectId)){
      user.permittedProjects.push(projectId)
    }
    user.save()
  })
}
